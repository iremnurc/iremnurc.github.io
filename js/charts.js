...
export function createXPChart(data, containerId) {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<p class="text-yellow-400 text-center py-8 font-mono">No XP data available</p>';
        return;
    }
    ...
    circlesData.forEach(({ x, y, item, index }) => {
        // Point circle with performance optimization
        const circle = createSVGElement('circle', {
            ...
            stroke: '#EBEFEF',
            'stroke-width': 2,
            class: 'point chart-point'
        });
        circle.style.cursor = 'pointer';
        
        // Create tooltip with performance-optimized classes
...
export function createSkillsRadarChart(skillsData, containerId) {
    ...
    if (!skillsData || skillsData.length === 0) {
        container.innerHTML = '<p class="text-yellow-400 text-center py-8 font-mono">No skills data available</p>';
        return;
    }
    ...
    skillsData.forEach((skill, index) => {
        ...
        const label = createSVGElement('text', {
            ...
            class: 'text-xs font-semibold chart-label',
            fill: '#F0EFEF',
            style: 'font-family: "JetBrains Mono", monospace;'
        });
    ...
    points.forEach((point, index) => {
        // Create tooltip for this point with performance optimization
        const tooltip = createSVGElement('g', {
            class: 'skill-tooltip chart-tooltip'
        });
        ...
        if (label) {
            ...
            label.addEventListener('mouseenter', () => {
                requestAnimationFrame(() => {
                    ...
                    label.setAttribute('fill', '#DDFF30');
                    tooltipRefs[index].classList.add('visible');
                });
            });
            ...
            label.addEventListener('mouseleave', () => {
                requestAnimationFrame(() => {
                    ...
                    label.setAttribute('fill', '#F0EFEF');
                    tooltipRefs[index].classList.remove('visible');
                });
            });
        }
    });
